...
import { authClient, getStoredAuthToken, extractTokenFromSession, updateUserProfile } from '@/lib/auth-client'
...
export async function getMyProfile(axiosSecure) {
  ...
  if (!axiosSecure) {
    ...
    let token = extractTokenFromSession(session) || getStoredAuthToken()
